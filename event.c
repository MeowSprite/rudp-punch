...
*/
static int
event_delete(struct event_data **firstp, int (*fn)(int, void*), void *arg, int fd) {
  struct event_data *e, **e_prev;

  e_prev = firstp;
  for (e = *firstp; e; e = e->e_next) {
    if (fn == e->e_fn && arg == e->e_arg && (fd == NULL || fd == e->e_fd)) {
      *e_prev = e->e_next;
      free(e);
      return 0;
    }
...
int event_timeout_delete(int (*fn)(int, void*), void *arg) {
  return event_delete(&ee_timers, fn, arg, NULL);
}
...
{
  return event_delete(&ee, fn, arg, NULL);
}

/*
* Deregister a file descriptor event.
*/
int event_fd_delete_fd(int (*fn)(int, void*), void *arg, int fd)
{
  return event_delete(&ee, fn, arg, fd);
}
    ...
    while (ee || ee_timers) {
        ...
        for (e=ee; e; e=e->e_next)
            if (e->e_type == EVENT_FD)
                FD_SET(e->e_fd, &fdset);    //put all the event_fd type socketfd into fdset

        if (ee_timers) {
            gettimeofday(&t0, NULL);
            timersub(&ee_timers->e_time, &t0, &t);    //left time
            if (t.tv_sec < 0)
                n = 0;
            else
                n = select(FD_SETSIZE, &fdset, NULL, NULL, &t);     //get the ready fd
        }
        else
