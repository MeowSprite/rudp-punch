...
/*
 * socket: local socketfd
 * seqno: seqno
 *  addr: reciver address struct
 */
void create_receiver_session(struct rudp_socket_list *socket, u_int32_t seqno, struct sockaddr_in *addr) {
  struct session *new_session = malloc(sizeof(struct session));
  if(new_session == NULL) {
...
/* input socket and returns a RUDP socket */
rudp_socket_t rudp_socket_from_socketfd(int sockfd) {
    if(rng_seeded == false) {
      srand(time(NULL));
      rng_seeded = true;
    }
...
int receive_callback(int file, void *arg) {
  ...
  recvfrom(file, &buf, sizeof(struct rudp_packet), 0, (struct sockaddr *)&sender, &sender_length);  //接受udp信息
  ...
  if(socket_list_head == NULL) {
    ...
    if((int)curr_socket->rsock == file) {
      ...
      if(curr_socket->sessions_list_head == NULL) {
        ...
        //找到session，如果是链表最后一个session就设置last_sessin
